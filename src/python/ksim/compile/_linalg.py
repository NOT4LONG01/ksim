"""GF(2) linear algebra for the error-basis transform.

Vendored verbatim from bloqade-tsim's ``tsim.utils.linalg.find_basis``
(Apache-2.0). tsim's ``matmul_gf2`` is *not* vendored — it is part of the
evaluator, which already lives in ``ksim.evaluate`` (numpy) and the GPU kernel;
ksim only needs the compile-time basis reduction here.
"""
import numpy as np
