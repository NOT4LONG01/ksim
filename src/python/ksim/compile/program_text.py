"""Conversion utilities between the T-gate shorthand dialect and stim program text.

Vendored verbatim from bloqade-tsim's ``tsim.utils.program_text`` (Apache-2.0).
Lets ``ksim`` accept the same convenience text (``T 0`` → ``S[T] 0`` etc.) that
``tsim.Circuit(text)`` did, so string input is a drop-in. Real ``stim.Circuit``
objects built by the project already use the expanded form and bypass this.
"""
