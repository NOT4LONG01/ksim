...
Vendored from bloqade-tsim (Apache-2.0); see ``ksim/__init__.py`` for provenance.
The only behavioural change vs. upstream: ``prepare_graph`` accepts a
``stim.Circuit`` directly instead of a ``tsim.circuit.Circuit`` wrapper.
"""
...
from .instructions import GraphRepresentation
from .parse import parse_stim_circuit
from .types import SamplingGraph
from ._linalg import find_basis
...
def prepare_graph(circuit: stim.Circuit, *, sample_detectors: bool) -> SamplingGraph:
